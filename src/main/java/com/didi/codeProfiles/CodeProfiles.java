...
public class CodeProfiles extends Applet implements Runnable {
    ...
    int programLineIndexAsWritten[] = new int[256];
    long programLineExecutionCount[] = new long[256];
    int programTrace[] = new int[4096], lastLineCallInProgram;
    Line programLines[] = null;
    Line lineDrawnLarger, lastLineDrawnLarger;
    double lineDrawnLargerRatio = 1. / 3.;
    //==============================================================================
    ...
    //   Tiny tweaks December 2024 were a conservation effort for new machines.   //
    //                                                                            //
    //   W. Bradford Paley                                         brad@didi.co   //
    //   Copyright (c) 2002, all rights reserved  related work at BradPaley.com   //
    //                                                                            //
    //============================================================================//
    Line lineToStartTraces = null;
    //==============================================================================
    int lineUnderMouseIndex;
    //==============================================================================
    int pc = 0, drawnPCWaitTargetLine = -1;
    int i11 = line(80);
    int i16 = line(132);
    int i1 = line(1);
    int i6 = line(42);
    ...
    int i3 = line(31);
    //==============================================================================
    Image cacheImage, offscreenImage;
    int i7 = line(54);
    Graphics cacheGraphics, offscreenGraphics;
    int i8 = line(55);
    boolean drawCacheNeeded = true, resetTraceVectors = false;
    int i9 = line(62);
    Vector pastPCPositions = null;
    int i13 = line(109);
    Vector pastFixationPositions = null;
    int i18 = line(177);
    Line lastFixationLine;
    int i21 = line(200);
    Vector pastInsertionPositions = null;
    int i19 = line(184);
    Line lastInsertionLine;
    int i20 = line(197);
    //==============================================================================
    int columnWidth;
    int i27 = line(249);
    ...
    void step() {
        ...
        Line lastLine = programLines[drawnFixationLine];
        line(206);
        ...
        if (drawnFixationPoint == -1) drawnFixationPoint =
                programLines[drawnFixationLine].s.length() - 2;
        line(94);
        drawnFixationPoint++;
        line(97);
        ...
        if (++drawnInsertionPoint >
                programLines[programLineIndexAsWritten[drawnInsertionLine] +
                        drawnInsertionLineBackup].s.trim().length()) {
            line(91);
            ...
            if (drawnInsertionLineBackup < 0) {
                ...
                do {
                    drawnInsertionLine = (++drawnInsertionLine) %
                            programLineIndexAsWritten.length;
                    line(84);
                ...
                do {
                    drawnInsertionLineBackup--;
                        ...
                        programLines[programLineIndexAsWritten[drawnInsertionLine] +
                                drawnInsertionLineBackup].hasLineCall == false);
                line(105);
        ...
        if (drawnPCWaitTargetLine == -1) {
            lastLine = programLines[programLineIndexAsWritten[
                    programTrace[drawnPC]]];
            line(207);
            drawnPC = (++drawnPC) % programTrace.length;
            line(74);
            ...
            if (drawnPC == 0) pc = -drawnInsertionLine * 10;
            line(99);
    ...
    public void run() {
        ...
        addMouseMotionListener(new MouseMotionAdapter() {
            public void mouseMoved(MouseEvent e) {
                lineDrawnLarger = findLineUnderMouse(e);
                line(116);
            }
        });
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                lineToStartTraces = findLineUnderMouse(e);
                line(248);
            }
        });
        ...
        while (go) {
            ...
            //  2024 conservation: 45 ms slows it down for today's faster machines
            try {
                Thread.sleep(Math.max(0,
                        55 - (System.currentTimeMillis() - lastUpdateTime)));
                line(70);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public Line findLineUnderMouse(MouseEvent e) {
        line(114);

        Line lineUnderMouse = null;
        line(119);
    ...
    //==============================================================================
    void brightenExecutedLines(Graphics g, boolean all) {
        line(135);

        //  loop through all lines as written and update their brightness
        for (int i = 0; i < programLineIndexAsWritten.length; i++) {
            line(140);

            //  calculate a new brightness level
            double newBrightness =
                    (Math.log((double) programLineExecutionCount[i] + 1));
            line(141);
            ...
            if (all || (programLines != null && (Math.abs(newBrightness -
                ...
                int index = programLineIndexAsWritten[i], backup = 0;
                line(137);
                ...
                int greenComponent = (int) Math.min(150,
                        70 + programLines[index].brightness * 2);
                line(149);
                g.setColor(
                ...
                if (programLines != null && programLines[index] != null) {
                    ...
                    do {
                        ...
                        backup--;
                            ...
                            programLines[index + backup].hasLineCall == false);
                    line(139);
                }
            }
        }
    ...
    Line[] readFile(String fileName) {
        ...
        try {
            ...
            //  2024 conservation: give it time to read
            try {
                Thread.sleep(10);
            } catch (Exception e) {
            }
        ...
        Line lines[] = new Line[v.size()];
        line(33);
        for (int i = 0; i < v.size(); i++) {
            lines[i] = (Line) v.elementAt(i);
            line(48);
            ...
            int lineCallPos = lines[i].s.indexOf("line(");
            line(85);
            int lineCallEndPos = lines[i].s.indexOf(")", lineCallPos);
            line(82);
            if (lineCallPos != -1 &&
    ...
    Line readLine(BufferedReader br) {
        line(15);
        int length = 0;
        line(20);
        int ch = 0;
    ...
    public synchronized void drawAll(Graphics g) {
        ...
        if (cacheImage == null ||
                ...
                size().height != cacheImage.getHeight(this)) {
            line(59);
        ...
        if (drawCacheNeeded) drawCache(cacheGraphics);
        line(66);
        ...
        offscreenGraphics.drawImage(cacheImage, 0, 0, this);
        line(67);
        ...
        if (resetTraceVectors) {
            line(229);
            pastPCPositions = new Vector();
            line(217);
            pastFixationPositions = new Vector();
            line(218);
            pastInsertionPositions = new Vector();
            line(219);
        ...
        Line lineRecentlyDrawnLarger = lineDrawnLarger;
        line(124);
        if (lineRecentlyDrawnLarger != null) {
            line(121);
            offscreenGraphics.setColor(new Color(30, 255, 30));
            line(120);
            offscreenGraphics.setFont(
                    new Font(getFont().getName(), getFont().getStyle(), 12));
            line(122);
            offscreenGraphics.drawString(lineRecentlyDrawnLarger.s, (int) Math.min(
                    ...
                    lineRecentlyDrawnLarger.x), lineRecentlyDrawnLarger.y);
            line(123);
        ...
        if (programLines != null) {
            line(113);
            offscreenGraphics.setColor(new Color(255, 255 * 7 / 8, 0));
            line(87);
        ...
        if (programLineIndexAsWritten[drawnInsertionLine] != 0) {
            ...
            Line insertionLine = programLines[programLineIndexAsWritten[
                    drawnInsertionLine] + drawnInsertionLineBackup];
            line(148);
            ...
            offscreenGraphics.drawString(
                    ...
                    (int) Math.min(size().width - lineDrawnLargerRatio *
                            ...
                            insertionLine.x), insertionLine.y);
            line(90);
            ...
            if (lastInsertionLine == null ||
                ...
                pastInsertionPositions.addElement(
                        new Point(insertionLine.x, insertionLine.y));
                line(199);
                pastInsertionPositions.addElement(
    ...
    void drawExecutionTrace(Graphics g) {
        ...
        for (int i = 1; i < pastPCPositions.size(); i++) {
            ...
            int lineBrightness = Math.min(140, 20 + i +
                    ...
                    Math.max(0, 200 - pastPCPositions.size()));
            line(211);
            ...
            offscreenGraphics.drawLine(
                    ...
                    ((Point) pastPCPositions.elementAt(i)).y);
            line(112);
        }
    }
    ...
    void drawInsertionTrace(Graphics g) {
        ...
        while (pastInsertionPositions.size() > 30)
            pastInsertionPositions.removeElementAt(0);
        line(186);

        if (pastInsertionPositions.size() > 1) {
            line(187);
            ...
            for (int i = 0;
                ...
                if (lastStepRatio < .5 && lastStepRatio != 0. &&
                    ...
                    lastStep = (int) ((lastStepRatio - .5) * 2 * steps);
                line(215);

                //  do the drawing
                Point p2, p1 = new Point(
                        (int) Math.round(insertionSpline[i][0].evaluate(0)),
                        (int) Math.round(insertionSpline[i][1].evaluate(0)));
                line(194);
    ...
    void drawFixationTrace(Graphics g) {
        ...
        while (pastFixationPositions.size() > 80)
            pastFixationPositions.removeElementAt(0);
        line(179);

        if (pastFixationPositions.size() > 1) {
            line(183);
            ...
            for (int i = 0; i < fixationSpline.length - 1; i++) {
                ...
                int lineBrightness = Math.min(120, 20 + (int) (i * 1.5) +
                        Math.max(0, 80 - fixationSpline.length) * 2);
                line(203);
                offscreenGraphics.setColor(
                ...
                Point p2, p1 = new Point(
                        ...
                        (int) Math.round(fixationSpline[i][1].evaluate(0)));
                line(191);
    ...
    public void drawCode(Graphics g, int w, int h) {
        line(41);
        int columns = 4, linesPerCol = (programLines.length + columns - 1) / columns,
                ...
                g.getFont()).stringWidth(" ") * 100) / (columns - 1);
        line(49);
        double lineHeight = (h - margin * 2) / (double) (linesPerCol + 2);
        line(50);
        ...
        for (int i = 0; i < programLines.length; i++) {
            ...
            if (programLines[i] != null) {
                ...
                programLines[i].x = margin + programLines[i].leadingPixels +
                        (i / linesPerCol) * colSpacing;
                line(76);
                programLines[i].y =
                        margin + (int) ((i % linesPerCol) * lineHeight);
                line(77);
                g.drawString(
                        programLines[i].s, programLines[i].x, programLines[i].y);
                line(47);
            }
        }
    ...
    //==============================================================================
    Cubic[][] calculateNaturalCubic(Vector v) {
        line(153);
        int i, n = v.size() - 1;
        line(159);
        double gammas[] = new double[n + 1];
        line(154);
        double deltaXs[] = new double[n + 1];
        line(155);
        ...
        Cubic coefficients[][] = new Cubic[n][2];
        line(172);
        for (i = 0; i < n; i++) {
